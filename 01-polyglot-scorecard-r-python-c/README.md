...
- **C does the deployed scoring hot-path** because a real
  ...
  possible; this repo measures that gap rather than asserting it: on the
  latest measured run the compiled engine is **278x faster than a
  pure-Python loop and 11.1x faster than vectorized NumPy** on the same
  lookup-and-sum operation (wall-clock throughput varies run to run with
  machine load — see Section 6 for the exact numbers), while reproducing
  R's exact scorecard output.

## Techniques used

- **WOE (Weight of Evidence) binning + Information Value**, hand-rolled
  in R with a monotonic-merge step and a special case for low-cardinality
  skewed count features (see the binning methodology note below).
- **Logistic regression scorecard with PDO (Points to Double the Odds)
  scaling** — the standard banking convention for converting a `glm`'s
  log-odds into an interpretable points scale.
- **Gradient-boosted trees** (XGBoost, LightGBM) and **Random Forest**
  as ML challengers, cross-validated with `StratifiedKFold`.
- **SHAP** (`TreeExplainer`/`LinearExplainer`) for challenger-model
  explainability, cross-checked against the scorecard's own IV ranking.
- **Deep learning**: a PyTorch MLP with **Focal Loss** (class-imbalance
  aware) and a controlled comparison of ReLU/GELU/Swish activations.
- **Reject inference** (Hard Cutoff, Parceling hard/soft) to correct the
  selection bias of training only on approved applicants.
- **Population Stability Index (PSI)** for score-drift monitoring.
- **Compiled C scoring engine** exposed to Python via `ctypes`, verified
  bit-for-bit against R's independently computed scorecard.
- **FastAPI service** putting the C-engine Champion and the ML
  Challenger behind the same low-latency endpoints (Champion/Challenger
  pattern).
- **DuckDB** for local, file-based, SQL-queryable metrics/predictions
  persistence across pipeline runs.
- **Interactive Plotly visualization** (self-contained HTML) of the
  score distribution by risk decile, alongside the static Matplotlib
  result figures.
...
│   └── visualization/
│       ├── plots.py
│       └── interactive_score_distribution.py  # Plotly HTML, score x risk decile
├── notebooks/
...
**Interactive version** (hover for per-decile bad rate, n, and score
range; zoom/pan): [score distribution by risk decile, test set](https://htmlpreview.github.io/?https://github.com/Rxyxs/credit-risk-scoring-lab/blob/main/01-polyglot-scorecard-r-python-c/outputs/interactive/score_distribution_by_risk_band.html)
— generated by `src/visualization/interactive_score_distribution.py`,
self-contained Plotly HTML (no server, no external JS).
...
| Throughput, C (ctypes) | **270,592,055 rows/sec** |
| Throughput, NumPy vectorized | 24,310,671 rows/sec |
| Throughput, pure Python loop | 972,886 rows/sec |
...
- **The C engine's latency advantage survives the trip through a real
  serving path**: 151x faster at the median and 83x at p99 than the ML
